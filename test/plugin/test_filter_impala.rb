...
class ImpalaFilterTest < Test::Unit::TestCase
  ...
  test "failure" do
    ...
    messages5 = [{ "message": "Unable to open scanner for node with id ‘0’ for Kudu table ‘n???.d??_so’ : Time out : exceeded configure scan timeout of 180.000s: Scan RPC to x.x.x.x:7050 timed out after 179.999 (ON_OUTBOUND_QUEUE)" }]
